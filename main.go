...
func (g *game) isValidMove(m move) (bool, string) {
	...
	switch {
		...
		case b1 && b4: return false, "not player's turn"
		case b2 && b3: return false, "not player's turn"
		case !b1 && !b2: return false, "not in the game"
	}
...
func (wh *webHandler) ServeHTTP (w http.ResponseWriter, req *http.Request) {
	...
	if req.Method == "POST" {
		...
		switch req.URL.Path {
			case "/game/stranger": webPollStranger(w, req, wh.dbChan)
			case "/game/friend": webFriendGame(w, req, wh.dbChan)
			case "/game/join": webJoinGame(w, req, wh.dbChan)
			default: {
...
func webPollStranger(w http.ResponseWriter, req *http.Request, dbChan chan message) {
	...
	w.SetHeader("Content-Type", "text/json")
...
func webFriendGame(w http.ResponseWriter, req *http.Request, dbChan chan message) {
	...
	switch rand.Int() % 2 {
		case 0: g.playerBlack = name
		case 1: g.playerWhite = name
	}
...
func webJoinGame(w http.ResponseWriter, req *http.Request, dbChan chan message) {
	...
	switch {
		case g.playerBlack == "": g.playerBlack = name
		case g.playerWhite == "": g.playerWhite = name
		default: {
...
func webPushMove(w http.ResponseWriter, req *http.Request, dbChan chan message) {
	...
	switch {
		...
		case playerName == g.playerBlack: opponentChan = g.whiteChan
		case playerName == g.playerWhite: opponentChan = g.blackChan
		default: {
